...
describe('translation engine', function () {
  ...
  xit('translate with google translate', async () => {
    ...
    const texts = await engine.translateHtml(['one', 'two', 'three']);
  ...
  xit('translate with gcloud translate', async () => {
    ...
    const texts = await engine.translateHtml(['a`b`c*d*<a href="e">e</a>']);
    expect(texts).toEqual([
      '一个`b` c *d* [e](e)\n',
    ]);
  });
